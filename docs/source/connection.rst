...
Sending a transaction to another peer via bluetooth, requires you to pair the devices via the Android bluetooth manager. After they are paired, the app will which devices your device is paired with. To initiate a transaction, press on one of the devices.
